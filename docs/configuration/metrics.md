...
| celeborn.metrics.master.clientMetrics.enabled | false | false | When true, the master re-exposes client-side metrics forwarded in application heartbeats on its Prometheus endpoint. Disabled by default because the metrics are per-label-set (not per-applicationId) and can produce unbounded cardinality if many distinct label combinations are used. Requires `celeborn.client.metrics.enabled` and non-empty `celeborn.client.metrics.appLabels` on the client side to have any effect. | 0.7.0 |  | 
